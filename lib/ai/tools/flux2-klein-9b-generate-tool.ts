...
interface Flux2Klein9BEditInput {
    prompt: string;
...
interface Flux2Klein9BReferenceInput {
    prompt: string;
...
async function normalizeReferenceImages(referenceImages?: string[]): Promise<string[] | undefined> {
    ...
    for (const image of referenceImages) {
        try {
            ...
            console.warn(`[FLUX.2 Klein 9B] Skipping reference image: ${message}`);
        }
    }
...
async function executeFlux2Klein9B(
    sessionId: string,
    input: Flux2Klein9BGenerateInput
): Promise<Flux2Klein9BGenerateOutput> {
    try {
...
const flux2Klein9BEditInputSchema = jsonSchema<Flux2Klein9BEditInput>({
    type: "object",
    title: "Flux2Klein9BEditInput",
    description: "Input for FLUX.2 Klein 9B local image editing",
    properties: {
        prompt: {
            type: "string",
...
const flux2Klein9BReferenceInputSchema = jsonSchema<Flux2Klein9BReferenceInput>({
    type: "object",
    title: "Flux2Klein9BReferenceInput",
    description: "Input for FLUX.2 Klein 9B reference-guided generation",
    properties: {
        prompt: {
            type: "string",
...
- Image editing: ~14-18 seconds`,
        ...
        execute: async (input): Promise<Flux2Klein9BGenerateOutput> => {
            return executeFlux2Klein9B(sessionId, input);
        },
    });
}

/**
 * Create the FLUX.2 Klein 9B edit tool
 * @param sessionId - Session ID for saving generated images to local storage
 */
export function createFlux2Klein9BEditTool(sessionId: string) {
    return tool({
        description: `Edit one or more images using local FLUX.2 Klein 9B model via ComfyUI.
...
- **guidance** (optional): CFG scale (default: 4.0)`,
        ...
        inputSchema: flux2Klein9BEditInputSchema,

        execute: async (input): Promise<Flux2Klein9BGenerateOutput> => {
            return executeFlux2Klein9B(sessionId, {
                prompt: input.prompt,
                seed: input.seed,
                width: input.width,
                height: input.height,
...
/**
 * Create the FLUX.2 Klein 9B reference tool
 * @param sessionId - Session ID for saving generated images to local storage
 */
export function createFlux2Klein9BReferenceTool(sessionId: string) {
    return tool({
        description: `Generate images guided by reference images using local FLUX.2 Klein 9B model via ComfyUI.
...
- **guidance** (optional): CFG scale (default: 4.0)`,
        ...
        inputSchema: flux2Klein9BReferenceInputSchema,
