...
import { resolveSeleneTemplateTools, type ToolWarning } from "@/lib/characters/templates/resolve-tools";

/**
 * GET /api/tools/resolve
 *
 * Returns the resolved tool configuration for the Selene default template
 ...
 *
 * Query params:
 * - templateId: Template ID to resolve (currently only "selene-default" supported)
 */
export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const templateId = searchParams.get("templateId") || "selene-default";

    if (templateId !== "selene-default") {
