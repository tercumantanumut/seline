...
export const settingsSectionShellClassName =
  "space-y-5 rounded-2xl bg-terminal-bg/5 p-3 dark:bg-terminal-cream-dark/25 sm:p-4";

export const settingsInputClassName =
  "w-full rounded-md border border-terminal-border/60 bg-terminal-cream/95 px-3 py-2 font-mono text-sm text-terminal-dark shadow-sm transition-colors placeholder:text-terminal-muted/75 focus:border-terminal-green focus:outline-none focus:ring-2 focus:ring-terminal-green/30 disabled:cursor-not-allowed disabled:opacity-60 dark:border-terminal-border/90 dark:bg-terminal-cream-dark/50 dark:text-terminal-dark dark:placeholder:text-terminal-muted";
...
export function SettingsPanelCard({ title, description, children, className }: SettingsPanelCardProps) {
  return (
    <section
      className={cn(
        "space-y-5 rounded-2xl border border-terminal-border/60 bg-terminal-cream/95 p-4 shadow-sm dark:border-terminal-border/90 dark:bg-terminal-cream-dark/70 sm:p-6",
        className
      )}
    >
...
export function SettingsOptionGroup({ title, description, children, className }: SettingsOptionGroupProps) {
  return (
    <section className={cn("space-y-3", className)}>
      <header className="space-y-1">
        <h4 className="font-mono text-sm font-semibold text-terminal-dark">{title}</h4>
        {description && <p className="font-mono text-xs leading-relaxed text-terminal-muted">{description}</p>}
      </header>
...
export function SettingsToggleRow({ id, label, description, checked, onChange }: SettingsToggleRowProps) {
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-terminal-border/55 bg-terminal-bg/5 p-3.5 dark:border-terminal-border/90 dark:bg-terminal-cream-dark/45 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <label htmlFor={id} className="font-mono text-sm font-semibold text-terminal-dark">
          {label}
        </label>
        {description && (
...
export function SettingsField({ label, htmlFor, helperText, children, className }: SettingsFieldProps) {
  return (
    <div className={cn("space-y-2", className)}>
      <label htmlFor={htmlFor} className="block font-mono text-sm font-semibold text-terminal-dark">
        {label}
      </label>
      {children}
      {helperText && (
    ...
    <label
      ...
      className={cn(
        ...
        checked
          ...
          : "border-terminal-border/55 bg-terminal-bg/5 hover:bg-terminal-bg/10 dark:border-terminal-border/90 dark:bg-terminal-cream-dark/45 dark:hover:bg-terminal-cream-dark/60"
      )}
    >
      <input
        id={id}
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-mono text-sm font-semibold text-terminal-dark">{label}</span>
          {badge && (
