# Flux2 Klein 9B: ComfyUI Docker Deployment with API

Flux2 Klein 9B is a containerized image generation pipeline using Black Forest Labs' FLUX.2 Klein 9B model. It combines ComfyUI with a FastAPI backend for both text-to-image generation and image editing tasks. This is the larger, higher-quality variant compared to the 4B model.
...
- **Text-to-Image Generation**: Generate images from text prompts with enhanced detail
...
- **Higher Quality**: More detailed outputs compared to 4B variant
...
**⚠️ Important:** The 9B model requires HuggingFace authentication. You must set the `HF_TOKEN` environment variable before building.
...
```bash
# Set HuggingFace token for model downloads (REQUIRED for 9B model)
...
# Check ComfyUI (port 8085)
curl http://localhost:8085

# Check API health (port 5052)
...
| Service | URL |
|---------|-----|
| **API Endpoint** | `http://localhost:5052/flux2/` |
| **API Docs** | `http://localhost:5052/docs` |
| **ComfyUI Interface** | `http://localhost:8085` |
...
```bash
curl -X POST "http://localhost:5052/flux2/generate-async" \
...
```bash
curl "http://localhost:5052/flux2/status/abc123-uuid" \
  -H "X-API-Key: internal-gateway-key"
```
...
|----------|---------|-------------|
| `COMFYUI_SERVER` | `http://flux2-klein-9b-comfy:8081` | ComfyUI server URL |
...
| `HF_TOKEN` | - | HuggingFace token (build time, **REQUIRED**) |

## Troubleshooting

### Common Issues

#### HuggingFace Authentication Error (Exit Code 6)
The 9B model requires authentication. Ensure `HF_TOKEN` is set:
```bash
export HF_TOKEN=your_huggingface_token
docker-compose build
```
...
```bash
# Check ComfyUI logs
docker logs flux2-klein-9b-comfy
...
```bash
# Check API logs
docker logs flux2-klein-9b-api

# Verify Redis connection
docker exec flux2-klein-9b-api python -c "import redis; r=redis.from_url('redis://api-redis-1:6379/0'); print(r.ping())"
```

#### GPU/CUDA Issues
```bash
# Verify GPU is accessible
docker exec flux2-klein-9b-comfy nvidia-smi
```

#### Out of Memory (OOM) Errors
The 9B model requires ~16GB+ VRAM. If you encounter OOM errors:
- Reduce batch size
- Use smaller image dimensions
- Consider using the 4B variant instead

### Performance

| Operation | Typical Time |
|-----------|--------------|
| Text-to-Image (1024x1024, 20 steps) | ~10-12 seconds |
| Image Editing (20 steps) | ~14-18 seconds |
...
### When to Use 9B vs 4B

- **Use 9B** for: Production quality, detailed images, when quality is priority over speed
- **Use 4B** for: Faster iterations, prototyping, lower VRAM GPUs, real-time applications
